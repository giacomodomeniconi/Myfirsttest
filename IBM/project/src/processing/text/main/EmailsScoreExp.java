	...
	public EmailsScoreExp() {
		...
		System.out.println("***** entities: "+ Arrays.toString(e.getAlchemy_entities_onlyStringList().toArray()));
	  	...
	  	for(int i=0;i<emails.size();i++){
			...
			double simcos_text=EmailsComparator.compareContent_CosineSimilarity(e, ei);
			double simcos_subj=EmailsComparator.compareContent_SubjectCosineSimilarity(e, ei);
			double jacc_subj=EmailsComparator.compareContent_SubjectJaccardSimilarity(e, ei);
			double simcos_concepts=EmailsComparator.compareContent_AlchemyConceptsCosineSimilairty(e, ei);
			double simcos_keywords=EmailsComparator.compareContent_AlchemykeywordCosineSimilairty(e, ei);
			double simcos_entities=EmailsComparator.compareContent_AlchemyEntitiesCosineSimilairty(e, ei);		
			
			double jacc_users=EmailsComparator.comparePeople_UsersJaccardSimilarity(e,ei);
			System.out.println(jacc_users);
			
			double time_dist=EmailsComparator.compareTime_logDistInDays(e, ei);
			text_sim_map.put(i, (1+simcos_text+simcos_subj+simcos_concepts+simcos_entities+simcos_keywords)*(1+time_dist)*(1+jacc_users));
		}
    	...
    	for(Entry<Integer,Double> entry : sortedPrediction.entrySet()){
    		...
    		System.out.println(Util.truncate(entry.getValue())+"\t"+email.getThreadid()+"\t"+email.getId()+"\t"+email.getSubject());
    	}
